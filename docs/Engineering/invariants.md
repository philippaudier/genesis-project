...
| 4 | The Simulation is **logically** deterministic (Level 1): identical initial state + identical inputs ⇒ an identical sequence of decisions and transformations. | Art. IV (Determinism) | **Test-time + convention.** A replay test (same seed/inputs ⇒ same logical state), plus conventions forbidding wall-clock reads, unseeded randomness, and undefined iteration order. *Logical* determinism is the target here — bit-identical numeric results (Levels 2–3) are a separate, stronger guarantee deferred to a future numeric-guarantees RFC. |
